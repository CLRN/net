...
template<typename Result, typename Tuple, typename ... RuntimeArg>
class ConcreteFactory : public IFactory<Result, RuntimeArg...>
{
public:
    ConcreteFactory(const Tuple& arg) : m_Tuple(arg) {}
    virtual boost::shared_ptr<Result> Create(const RuntimeArg& ... runtime) override
    {
        return boost::make_shared<Result>(runtime..., m_Tuple);
    }
private:
    const Tuple m_Tuple;
};
...
{
    typedef std::tuple<Static...> Tuple;
    return boost::make_shared<ConcreteFactory<Result, Tuple, Runtime...>>(std::forward_as_tuple(args...));
}
